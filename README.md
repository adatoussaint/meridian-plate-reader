...
Then **Run All**. Parts A through H run entirely on CPU with no GPU or large downloads required — this covers the full pipeline (segmentation, preprocessing, the local MLP classifier, and batch testing across noise conditions) and is enough to run the live demo. The business-cost analysis lives in `docs/Business_Note.docx`, not in the notebook.
...
- `ML_Final_Project.ipynb` — the full notebook: pipeline build, both models (MLP and CNN), and evaluation.
